...
//   const { userLoggedIn, updateUserLoggedIn } = useAuth(); // Add updateUserLoggedIn
...
//         updateUserLoggedIn(true); // Update userLoggedIn state
...
import * as React from "react"
import { Link, useNavigate } from "react-router-dom"
import { doSignInWithEmailAndPassword, doSignInWithGoogle, doPasswordReset } from "../../../Backend/Auth/auth"
import { useAuth } from "../Context/authContext/context"
import { UserContext } from "../Context/UserContext"
import { usePhotoStore } from "../store/photo.store"

export default function Login() {
  const { userLoggedIn, updateUserLoggedIn } = useAuth()
  ...
  const handleLogin = async (e) => {
    ...
    if (!isSigningIn) {
      ...
      try {
        ...
        // Update photo store directly as well
        setCurrentUser({
          id: user.uid,
          name: user.displayName || email.split("@")[0],
          email: user.email,
  ...
  return (
    <div
      className="flex justify-center items-center min-h-screen bg-gray-100 dark:bg-gray-900 p-6 relative py-24"
      style={{
        backgroundImage: "url('https://i.pinimg.com/736x/11/d9/76/11d976452ca684b3e7c0306cc306c3e1.jpg')",
        backgroundSize: "cover",
        backgroundPosition: "center",
      }}
    >
      ...
      <div className="relative w-full max-w-4xl bg-white dark:bg-gray-800 rounded-3xl shadow-2xl flex overflow-hidden transform scale-105">
        ...
        <div className="w-full md:w-1/2 p-16 flex flex-col justify-center">
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white text-center">
            Welcome to <span className="text-green-600 dark:text-green-400">EnFauna</span>
          </h1>
          <p className="text-gray-600 dark:text-gray-300 text-center mt-3">Please enter your details to sign in.</p>

          <form onSubmit={handleLogin} className="mt-6">
            <div className="mb-5">
              <label className="font-medium text-gray-900 dark:text-gray-300">Email</label>
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
            ...
            <div className="mb-5">
              <label className="font-medium text-gray-900 dark:text-gray-300">Password</label>
              <div className="relative">
                <input
                  type={showPassword ? "text" : "password"}
                  value={password}
            ...
            <div className="text-right mb-5">
              <Link to="/forgot-password" className="text-gray-500 dark:text-gray-400 text-sm">
                Forgot Password?
              </Link>
            </div>

            {errorMessage && (
              <p className="text-red-600 dark:text-red-400 font-semibold text-md text-center mb-2">{errorMessage}</p>
            )}
          ...
          <div className="mt-6 text-center">
            <p className="text-gray-500 dark:text-gray-400">
              Don't have an account?
              <Link to="/signup" className="text-green-600 dark:text-green-400 font-bold ml-1">
                Create Account!
              </Link>
            </p>
          </div>
